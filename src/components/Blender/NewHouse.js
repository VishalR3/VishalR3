...
const NewHouse = () => {
  ...
  useEffect(() => {
    ...
    gltfLoader.load("/models/newHouse.glb", (gltf) => {
      ...
      gltf.scene.traverse((child) => {
        if (
          ...
          if (child.material.name === "GlassPane") {
            child.material.transparent = true;
    ...
    directionalLight.position.set(11.347, 9.179, 12.431);
    ...
    // gui.add(directionalLight.shadow.camera, "zoom").min(-1).max(1).step(0.001);
    ...
    camera.position.x = 3.767;
    camera.position.y = 7.19;
    camera.position.z = 10.263;
    scene.add(camera);
